##  Finetuning Data
We use SatQuery-Instruct to finetune our model. The instruction following dataset is present in SatQuery_Instruct.json and the images are present in the [huggingface repo](https://huggingface.co/datasets/MBZUAI/SatQuery_Instruct). The images are split into multiple files. Download the separate files in the same folder and run the following script to merge them. 
...
| Data file name | Size |
| --- | ---: |
| [SatQuery_Instruct](https://huggingface.co/datasets/MBZUAI/SatQuery_Instruct/blob/main/SatQuery_Instruct.json) | 263 MB |
