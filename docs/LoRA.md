...
## Demo (Web UI)
You need SatQuery-7B to run the demo locally. Download the model from [SatQuery-7B](https://huggingface.co/MBZUAI/satquery-7B). After loading the model, run this command by giving the model path to launch the gradio demo.
#### Launch the demo
```Shell
python satquery_demo.py --model-path /path/to/model 
```

## Training

Please see sample training scripts for [LoRA](https://github.com/mbzuai-oryx/SatQuery/blob/main/scripts/finetune_lora.sh)
