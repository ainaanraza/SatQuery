# Customize Components in SatQuery

This is an initial guide on how to replace the LLMs, visual encoders, etc. with your choice of components.

## LLM

It is quite simple to swap out LLaMA to any other LLMs.  You can refer to our implementation of [`SatQuery_llama.py`](https://github.com/mbzuai-oryx/SatQuery/blob/main/satquery/model/language_model/satquery_llama.py) for an example of how to replace the LLM.
...
In `forward` function, you can see that we call `self.prepare_inputs_labels_for_multimodal` to process the multimodal inputs.  This function is defined in `SatQueryMetaForCausalLM` and you just need to insert it into the `forward` function of your LLM.
