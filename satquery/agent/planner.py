    ...
    def generate_plan(self, state: AgentState):
        ...
        if op == "metadata_query":
            for img in state.inputs:
                plan.append(
                    ToolCall(
                        tool_name="raster.metadata",
                        arguments={"path": img.path}
                    )
                )

        elif op == "change_vqa" and len(state.inputs) >= 2:
            img_a = state.inputs[0]
            img_b = state.inputs[1]

            plan.append(
                ToolCall(
                    tool_name="temporal_alignment",
                    arguments={
                        "image_a": img_a,
                        "image_b": img_b
                    }
            ...
            plan.append(
                ToolCall(
                    tool_name="semantic_change",
                    arguments={
                        "label_t1": "semantic_labels_t1",
                        "label_t2": "semantic_labels_t2"
                    }
                )
            )
            ...
            plan.append(
                ToolCall(
                    ...
                    arguments={
                        "optical_image": state.inputs[0],
                        "sar_image": state.inputs[1]
                    }
                )
            )
            ...
            plan.append(
                ToolCall(
                    tool_name="vision.answer",
                    arguments={
                        "question": state.query,
                        "image": state.inputs[0] if state.inputs else None
                    }
                )
            )
